...
if __name__ == "__main__":
    ...
    if os.environ.get("RENDER"):
        ...
        if bot_token:
            ...
            webhook_loop = None
            webhook_executor = concurrent.futures.ThreadPoolExecutor(max_workers=1)
            
            def setup_webhook_loop():
                global webhook_loop
                webhook_loop = asyncio.new_event_loop()
                asyncio.set_event_loop(webhook_loop)
                webhook_loop.run_until_complete(application.initialize())
                return webhook_loop
            
            # Initialize the webhook loop
            future = webhook_executor.submit(setup_webhook_loop)
            webhook_loop = future.result()
            ...
            def webhook():
                ...
                try:
                    ...
                    # Submit the update processing to the persistent event loop
                    def process_update():
                        if webhook_loop is not None:
                            return asyncio.run_coroutine_threadsafe(
                                application.process_update(update), 
                                webhook_loop
                            ).result()
                        else:
                            raise RuntimeError("Webhook loop not initialized")
                    
                    # Process in the webhook executor
                    webhook_executor.submit(process_update).result(timeout=30)
